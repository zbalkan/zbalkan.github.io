...
### Visual guides to NTDS.dit extraction and analysis

[TrustedSec's DIT Explorer blog post](https://trustedsec.com/blog/exploring-ntds-dit-part-1-cracking-the-surface-with-dit-explorer) includes screenshots of the tool's interface for browsing the directory hierarchy and extracting credentials interactively. The [Semperis NTDS.dit extraction guide](https://www.semperis.com/blog/ntds-dit-extraction-explained/) provides diagrams of the encryption chain, threat-actor profiles, and detection signatures. The [Netwrix NTDS.dit extraction article](https://blog.netwrix.com/2021/11/30/extracting-password-hashes-from-the-ntds-dit-file/) covers the attack vectors, file-locking mechanics, and why the offline-tooling landscape fragmented around CSVs and Python scripts.

## ditjson reads the database in one self-contained binary

[`ditjson`](https://github.com/zbalkan/ditjson) is a fork and continuation of `dumpntds` that folds the relevant `ntdsxtract` processing back into the executable. It opens the ESE database, classifies objects by `sAMAccountType`, resolves the relationships, decrypts credentials against the `SYSTEM` hive when supplied, and emits one JSON document. There are no CSVs, no Python, and nothing to install: it publishes as a single self-contained `.exe` that you copy in, run, and delete.

Bound the scope before the usage, because a tool that overstates itself is worse than one that understates. `ditjson` reads an offline `ntds.dit`; it does not acquire one, and it is not an attack tool — it has no DCSync, no live-DC interaction, no cracking. It is Windows-only by construction: the ESE access goes through Microsoft's ManagedEsent, which P/Invokes the Windows `esent.dll`, so there is no Linux build regardless of .NET's portability. It emits exactly one format, JSON, on standard output or to a file, and it does not blindly dump every ESE column — it extracts the supported users, groups, computers, decoded attributes, and relationships, and stops. If you need a full raw ESE export, this is the wrong tool and `libesedb` is the right one.
