---
title: "Reading an offline ntds.dit with a single Windows binary"
tags:
  - Active Directory
...
The Slack ping arrived before the command had finished running. A red-team pair, three days into an authorised internal engagement, had reached Domain Admin and gone for the obvious objective: pull `ntds.dit` and the `SYSTEM` hive off a domain controller with `ntdsutil`, take them offline, and demonstrate how many of the client's passwords crack in an afternoon. The `ntdsutil` command had not yet returned when the client's Wazuh manager lit up with a level-12 alert — *Possible NTDS.dit file extraction using ntdsutil.exe* — sourced from a Sysmon process-creation event on that exact controller. The blue team saw the grab in seconds. What they would never see was the next four hours.

That asymmetry is the whole subject of this post. Copying `ntds.dit` off a domain controller and decoding what is inside it are separate problems. The first one your sensors can see. The second happens offline, on the operator's own kit, and produces no telemetry at all. The engagement above proves both halves at once: the acquisition tripped a rule that fired instantly, and the offline hash extraction that followed left no trace on the client's estate, because there was nothing left on the client's estate to trace. A detection programme that does not understand the split will invest in watching the silent half.

The primary reader here is a detection engineer, and the fired rule is the thread we follow to a deployable version of itself. A pentester who needs to read the file they just pulled — on a hardened laptop with no Python and no appetite for standing up a toolchain — is the other reader, and the back half of the post is written for them. A Windows administrator who has never opened an ESE database will follow all of it, and should, because the person who runs the directory is usually the first to be asked what an anomalous `ntds.dit` copy means.

## ntds.dit is an ESE database, not a Microsoft black box

The `.dit` extension suggests something proprietary and sealed. It is not. `ntds.dit` is a single-file database in Microsoft's Extensible Storage Engine format — [ESE, formerly JET Blue](https://github.com/microsoft/Extensible-Storage-Engine), the same engine behind Exchange mailbox stores and the Windows Search index. One file holds the entire directory: tables, indexes, and a transaction log discipline that makes it a real database rather than a flat dump. That is why it cannot simply be read with a hex editor and grep. It is paged, B-tree indexed, and, for the parts that matter, encrypted.
...
The rule the engagement tripped is functionally this one; I have moved its ID into the `100000`+ local range so it cannot collide with a future upstream ruleset, which is the only change worth making to it as written. What is not worth pretending is that it is sufficient. A case-insensitive match on the string `ntdsutil` in a command line is the floor of this detection, not its ceiling, and every acquisition vector from the list above walks around it. Rename the binary and the regex misses. Use the `vssadmin`, `esentutl`, or raw-handle path and `ntdsutil` never appears. Run DCSync and no process is created on the DC at all. The rule earns its place because the loud, default `ntdsutil` IFM technique is genuinely common and this catches it cheaply — but it is one condition, and the `4688` command-line variant, the ESENT application-log rule, and a Sysmon `EventCode=11` rule for `ntds.dit` written outside `%SystemRoot%\NTDS` each cover a gap the string match leaves open. A tool-name regex is a tripwire for the careless, not a control against the deliberate.  The payoff of the acquisition/analysis split is a rule of thumb worth stating plainly. Nothing you run against the file afterwards — not `ditjson`, not `secretsdump`, not `ntdsxtract` — will show up on the domain controller. By the time the file is being read, the detection opportunity has already passed. An `ntds.dit` copy resting anywhere other than the live directory path is not a lead to investigate; it is an incident to declare.

For the detection half, [MITRE ATT&CK's DET0586](https://attack.mitre.org/detectionstrategies/DET0586/) defines the observables for `ntds.dit` extraction, and [AN1611](https://attack.mitre.org/analytics/AN1611/) covers the process-creation, file-creation, and VSS-activity data components. The [SigmaHQ rule for ESENT abuse](https://github.com/SigmaHQ/sigma/blob/master/rules/windows/builtin/application/esent/win_esent_ntdsutil_abuse.yml) provides a source file for the application-log detection that bypasses command-line obfuscation. Translating these to local rules — as the Wazuh rule above does — is the practitioner's work; the detections themselves are the framework.

## Offline, the file still has to be read, and the tooling aged badly

Back on the engagement, the operators had what they came for: `ntds.dit` and the matching `SYSTEM` hive on the red-team laptop, acquired under a signed scope, the alert already noted for the report's detection-recommendations section. The blue half of the story is over. The offline half — turning two files into a list of crackable hashes — is where the tooling has aged badly.

The canonical open-source answer for a decade was a two-stage pipeline. BSI's [`dumpntds`](https://github.com/bsi-group/dumpntds) opened the ESE database and wrote out the columns that mattered as `datatable.csv` and `linktable.csv`, sparing analysts a full raw dump of every ESE column. Csaba Barta's [`ntdsxtract`](https://github.com/csababarta/ntdsxtract) then took those CSVs and did the real work: resolving objects, joining the link table, decrypting hashes against the `SYSTEM` hive, parsing history and supplemental credentials. It worked, and it still teaches the internals better than anything. It also means a Python 2-era toolchain, two intermediate files written to disk holding decrypted material, and a multi-command dance to get one answer.

The other pole is Impacket's `secretsdump.py`, which fused acquisition and analysis into one tool and became the default. It reads the offline pair directly and dumps hashes in one line — and it is excellent — but it carries a full Python and Impacket install, which is exactly the dependency you do not want to stand up on a hardened, offline evidence workstation. Between the two sits a smaller field of GUI and single-purpose readers. [TrustedSec's DIT Explorer](https://github.com/trustedsec/DitExplorer) lets you browse the database as a hierarchical tree and inspect attributes visually, which is useful when you want to understand the structure before writing queries or simply need to spot an object without command-line scripting. The tradeoff is that a GUI reader gives you one thing at a time, where `ditjson` to JSON to `jq` gives you the whole export in any shape you ask for.  The through-line is that reading an offline directory should not require a runtime install, two intermediate credential-bearing files, or a scripting environment on a box you are trying to keep minimal. That is the specific problem the next section addresses, and the only problem it addresses.

Here' it would be unnecessary for me to write another tutorial for other ways of extraction, therefore, it is better to mention reputable resources instead. [TrustedSec's DIT Explorer blog post](https://trustedsec.com/blog/exploring-ntds-dit-part-1-cracking-the-surface-with-dit-explorer) includes screenshots of the tool's interface for browsing the directory hierarchy and extracting credentials interactively. The [Semperis NTDS.dit extraction guide](https://www.semperis.com/blog/ntds-dit-extraction-explained/) provides diagrams of the encryption chain, threat-actor profiles, and detection signatures. The [Netwrix NTDS.dit extraction article](https://blog.netwrix.com/2021/11/30/extracting-password-hashes-from-the-ntds-dit-file/) covers the attack vectors, file-locking mechanics, and why the offline-tooling landscape fragmented around CSVs and Python scripts.
...
[`ditjson` 2.0 was released on 11 August 2026](https://github.com/zbalkan/ditjson/releases). That is the version described here: it folds the relevant `ntdsxtract` processing back into the executable, opens the ESE database, classifies objects by `sAMAccountType`, resolves the relationships, decrypts credentials against the `SYSTEM` hive when supplied, and emits one JSON document. There are no CSVs, no Python, and no Impacket package or separate supporting runtime to install: the release is a single self-contained `.exe` that you copy in, run, and delete. Self-contained does not mean small — the executable bundles the .NET 10 runtime — but its size buys a predictable offline tool with no dependency setup. The default structured export needs no table or object-selection switches: it always extracts the supported users, groups, and computers.

Bound the scope before the usage, because a tool that overstates itself is worse than one that understates. `ditjson` reads an offline `ntds.dit`; it does not acquire one, and it is not an attack tool — it has no DCSync, no live-DC interaction, no cracking. The portability trade-off is deliberate: `ditjson` is Windows-only because `Microsoft.Database.ManagedEsent` ultimately calls the Windows `esent.dll`, so there is no Linux build regardless of .NET's portability. In return, a Windows analysis host needs no Python environment, Impacket installation, or package-management access; the one downloaded binary is the complete user-space tool. Its normal output does not blindly dump every ESE column: it extracts supported users, groups, computers, decoded attributes, and relationships. When an investigation really does need the underlying records, `--all` adds every table and column to that same JSON document.
...
The single design decision that makes it composable is stream discipline. Standard output carries only the JSON document. Every status line, warning, and error goes to standard error. The two streams redirect independently, so the document survives a pipe untouched while you still see progress on the console:
...
The internals are easier to trust when you can reproduce them against a file with known contents. Didier Stevens published a [practice `ntds.dit` and its matching `SYSTEM` hive](https://blog.didierstevens.com/2016/07/12/practice-ntds-dit-file-part-1/) for exactly this — a throwaway Windows Server 2003 forty-user forest. The vintage is deliberate on his part and convenient on ours: Server 2003 still stores LM hashes for short passwords and uses the RC4 `PEK` path, so this fixture exercises the legacy branch end to end. A modern DC would exercise the AES branch and populate Kerberos keys instead.
...
Run `ditjson` over the pair and let the diagnostics stream to the console while the JSON lands in a file. The status lines narrate the crypto chain from the theory section — one boot key out of `SYSTEM`, one `PEK` decrypted, then the hashes:
...
Where the design pays off is the query. Because the output is one JSON document, `jq` does the analysis the old pipeline needed a Python script for. The combined users-and-computers view — account name, NT hash, LM hash, dropping any account with neither — is one expression:
...
From there the queries become audit questions. `jq` is available on Windows via Chocolatey, the official `.exe` download, or any standard package manager, and it is valuable enough that any IT professional should have it in their toolkit — it turns JSON into structured answers without leaving the command line. Which accounts still carry an LM hash — a stored-credential weakness that should not survive a modern password policy — is a filter and a list:
...
The pivots a responder actually wants are one filter each. A password-change
...
Accounts created inside a suspected compromise window, which is where a
persistence account hides among legitimate churn:
...
The value is a collapse of a multi-tool, multi-file, runtime-dependent workflow into one artefact. A single Windows executable, copied to an offline analysis box, produces one machine-readable document that `jq`, PowerShell, or any SIEM ingestion path consumes without a parser. Nothing decrypted is written to an intermediate CSV that then has to be tracked and shredded; the one output file is the one thing to secure and destroy. For a red-team report, an IR triage, or a migration audit, that is fewer moving parts and fewer places for credential material to leak.

The catch is threefold, and stating it is not optional. First, it is Windows-only and always will be while it depends on the Windows ESE API, so a Linux-only evidence workflow stays on Impacket. Second, it reads offline files and does nothing live — it is not a substitute for `secretsdump`'s DCSync path when a lawful remote dump is what the engagement calls for. Third, and this outranks the other two: `ntds.dit`, the `SYSTEM` hive, and the JSON `ditjson` writes are all reusable credential material. The output is not a report you can attach to a ticket; it is a secret to be handled, access-controlled, and destroyed on the same footing as the database it came from. The warning in the project's README is there because the easiest way to cause the breach you were investigating is to leave its evidence lying around.

The applicable line to leave with is the one the whole post turns on. Detection belongs at acquisition, because analysis is silent — so wire `ntdsutil`, shadow-copy, and ESENT-event telemetry to fire on the copy, treat any `ntds.dit` found off its directory path as a declared incident rather than a lead, and keep a single self-contained reader on the analysis box so that when the file does land in your hands, reading it is the fast part.
