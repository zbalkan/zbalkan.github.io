...
The official [Wazuh 4.x rule syntax documentation](https://documentation.wazuh.com/4.14/user-manual/ruleset/ruleset-xml-syntax/rules.html) contains a complete list of information. However, this syntax reference documents every attribute and element as a separate item. This format helps users who are looking for specific details. It is less effective for users who want to understand how the language functions as a whole.
...
This terminology is new as of Wazuh 4.x and does not replace previous syntax. It is a way to interpret existing XML. This approach is not limited to Wazuh; similar concepts appear in Sigma, YARA-L, Elastic Security, Splunk SPL, and Microsoft Sentinel KQL, which I will briefly discuss after explaining the Wazuh 4.x model. These systems use different syntax, but all detections require descriptions and defined conditions to identify relevant events. Wazuh 4.x includes a feature not present in earlier versions: its temporal conditions count matches generated by other rules, rather than counting the logs directly.
...
This rule functions differently from others. It does not monitor single Windows error events. Instead, it monitors patterns that develop over a period of time. Rule `60011` must match eight times within a 240-second window for rule `60014` to trigger.
...
The following comparisons are analogies for readers who are familiar with one of these languages. They are not complete translations. The Wazuh model does not rely on these comparisons.
...
Once those two views are combined, an unfamiliar Wazuh 4.x ruleset becomes easier to approach. Locate the rule within the graph and open the node to determine what the rule describes and what conditions make it true. These questions are also applicable when writing a new rule, as the answers define its requirements. The second answer tells you whether the rule can be evaluated from the current event or needs state retained from earlier matches. Elastic Security, Splunk and Microsoft Sentinel draw the same boundary in their own vocabulary, mostly over raw events rather than over rule matches.
