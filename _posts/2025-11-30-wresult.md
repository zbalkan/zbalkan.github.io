...
tags:
  ...
  - Reporting
galleryConsole:
  - url: /assets/images/wresult-console.gif
    image_path: /assets/images/wresult-console.gif
galleryFile:
  - url: /assets/images/wresult-file.gif
    image_path: /assets/images/wresult-file.gif
galleryCollapsed:
  - url: /assets/images/wresult-collapsed.png
    image_path: /assets/images/wresult-collapsed.png
galleryExpanded:
  - url: /assets/images/wresult-expanded.png
    image_path: /assets/images/wresult-expanded.png
---

Security tools are only as effective in technical capabilities as their configuration. In environments managed with [Wazuh](https://wazuh.com/?utm_source=ambassadors&utm_medium=referral&utm_campaign=ambassadors+program), understanding what's actively applied on an agent isn't always straightforward. Wazuh's configuration model is layered: settings come from local files, central manager policies, and condition-based overrides. This flexibility introduces complexity-especially when you need to prove what the agent is doing.
...
- `agent.conf` (centralised policy, with conditional logic)
...
{% include gallery id="galleryConsole" caption="Print to console" %}
...
{% include gallery id="galleryFile" caption="Export to HTML file" %}

See the collapsed tree of configuration items.

{% include gallery id="galleryCollapsed" caption="HTML report with collapsed items" %}

Click "Show all" to have a broader view.

{% include gallery id="galleryExpanded" caption="HTML report with expanded items" %}

## A Note on Limitations (and What I'd Prefer)

This tool simulates agent behaviour. That means it's only as accurate as our understanding of how the agent resolves and applies configuration logic. If Wazuh changes its internals, this tool may lag behind until updated. That's always a risk with external parsers.
