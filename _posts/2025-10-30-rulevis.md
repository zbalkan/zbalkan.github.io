...
Once it runs, RuleVis spins up a local Flask app, parses the XMLs, builds the graph in memory, and opens the browser automatically. Logs go where you'd expect: `%LocalAppData%` on Windows, `~/Library/Logs` on macOS, and `~/.local/state` or `$XDG_STATE_HOME` on Linux.
