...
Between these poles sits a wider field than the two-stage CSV pipeline alone. [TrustedSec's DIT Explorer](https://github.com/trustedsec/DitExplorer) lets you browse the database as a hierarchical tree and inspect attributes visually. [DSInternals](https://github.com/MichaelGrafnetter/DSInternals), Michael Grafnetter's PowerShell module, is the deepest tool in the category by a wide margin: it reads live over MS-DRSR as well as offline, writes back into `ntds.dit` — password resets, SID History injection, group membership changes — and pulls LAPS, BitLocker, and DPAPI material out of the same file, though its own documentation warns plainly that improper use can cause irreversible damage to a domain controller. [`ntdissector`](https://github.com/synacktiv/ntdissector), from Synacktiv, is the closer sibling: a Python tool built on `dissect.esedb` that decrypts the full database to JSON filtered by object class and ships a BloodHound conversion script, at the cost of a dependency chain `ditjson`'s single `.exe` doesn't need. [NtdsAudit](https://github.com/dionach/NtdsAudit), from Dionach, stays narrower still — CSV and pwdump output with a wordlist-matching option for a same-run weak-password count, no JSON export or timeline. None of these three appeared in an earlier draft of this post, and that was an omission: `ditjson` doesn't out-capability any of them, it earns its place by being the smallest thing that produces one clean, timeline-capable JSON document with nothing to install.
...
`ditjson` is not a new project. [When I first shared it](https://www.linkedin.com/posts/zbalkan_github-zbalkanditjson-exports-all-ntdsdit-activity-7158917801674518529-NO3V), the proposition was deliberately small: point it at an exported `ntds.dit` and receive one JSON file; that first version also allowed selected table exports. The idea belongs to [Lars Karlslund](https://github.com/lkarlslund), the developer of the excellent [Adalanche](https://github.com/lkarlslund/Adalanche) Active Directory analysis tool, as he was looking for something that can be used as an input to Adalanche from `ntds.dit` file. The `ditjson` is my implementation of his idea, begun as a fork of `dumpntds`. Both the idea and the wider body of AD work behind it deserve explicit acknowledgement. This release returns to that original aim and carries it further rather than presenting the tool as something invented today.
