...
The one construct worth stating precisely is accumulated invalid time: the total duration during which a rule's declared hard dependencies went unsatisfied, weighted by the rule's criticality if needed. It requires timestamped validity transitions or a retained time series rather than a point-in-time snapshot, but it is rule-relative by construction and turns "the rule went silent" into "the rule was structurally unable to fire for this long."
